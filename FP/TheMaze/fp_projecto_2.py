...
#POSICAO, pela chave vida cujo valor e um natural, pela chave forca cujo valor
#e um natural e por ultimo pela chave exercito cujo valor e uma string nao
#vazia
...
def cria_unidade(p, v, f, string):
    ...
    Excepcao: Caso os argumentos nao sejam uma posicao, um natural, um natural,
    e uma string respectivamente, e gerado um erro de valor
    """    
...
#Para representar o ADT MAPA foi definido o tipo de dados embutido lista
#constituida por quatro tuplos sendo o primeiro as dimensoes (Nx, Ny) do
...
def eh_conj_paredes_interiores(d, arg):
    ...
    de ser um conjunto de posicoes de paredes interiores ao labirinto.
...
def eh_posicao_corredor(m, p):
    ...
    Assinatura da funcao: mapa x posicao -- booleano
    """            
    if (p not in m[1]) and (0 < obter_pos_x(p) < obter_pos_x(obter_tamanho(m))
        and (0 < obter_pos_y(p) < obter_pos_y(obter_tamanho(m)))):
        return True
    else:
        return False
...
def eh_posicao_parede(m, p):
    ...
    Assinatura da funcao: mapa x posicao -- booleano
    """            
    if not eh_posicao_corredor(m, p):
        return True
    else:
        return False
...
def mapa_para_str(m):
    ...
    posicoes_e1 = ()
    posicoes_e2 = ()
    #Obtem as posicoes das unidades de cada exercito separademente
    for unidade in e1:
    ...
    labirinto = coluna
    labirinto_externo = ''
    #Reproduz externamente o labirinto
...
def calcula_pontos(m, string):
    """Calcula os pontos de vida de todos as unidades em combate calculando a
...
def simula_turno(m):
    """Simula um turno na batalha entre os dois exercitos.
    
    Assinatura da funcao: mapa -- mapa
    """      
    todas_unidades = obter_todas_unidades(m)
    #Escolhe uma unidade de um exercito qualquer
    indice = 0
    while indice != (len(todas_unidades) - 1):
        if obter_inimigos_adjacentes(m, todas_unidades[indice]) != ():
            #Se existe inimigo em posicao adjacente realiza o ataque
            if unidade_ataca(todas_unidades[indice], 
                             obter_inimigos_adjacentes(m, 
                             todas_unidades[indice])[0]):
                eliminar_unidade(m, todas_unidades[indice])
        else:    
            #Move a unidade para a posicao adjacente imediata em ordem de
            #leitura em direcao a posicao objectivo
            mover_unidade(m, todas_unidades[indice],
                          obter_movimento(m, todas_unidades[indice]))
        indice += 1
    return m        

def simula_batalha(string, logico):
    f = open('config.txt', 'r')
    linhas = f.readlines()
    e1 = e2 = ()
    posicoes_unidades_e1 = linhas[4]
    posicoes_unidades_e2 = linhas[5]
    for posicao in linhas[4]:
        e1 = e1 + (cria_unidade(posicao, linhas[1][1], linhas[1][2],
                               linhas[1][0]))
    for posicao in linhas[5]:
        e2 = e2 + (cria_unidade(posicao, linhas[2][1], linhas[2][2],
                                linhas[2][0]))         
    m = cria_mapa(linhas[0], linhas[3], e1, e2) 
    while e1_pontuacao > 0 or e2_pontuacao > 0 or (m == simula_turno(m)):
        if logico == False:
            e1_pontuacao = calcula_pontos(m, obter_exercito(e1[0]))
            e2_pontuacao = calcula_pontos(m, obter_exercito(e2[0]))
            return '[' +  obter_nome_exercitos(m)[0] + ':' + e1_pontuacao + \
                   ' ' + obter_nome_exercitos(m)[1] + ':' + e2_pontuacao + \
                   ']' '\n' + simula_turno(m)
        else:
            e1_pontuacao = calcula_pontos(m, obter_exercito(e1[0]))
            e2_pontuacao = calcula_pontos(m, obter_exercito(e2[0]))
            return simula_turno(m) + '[' +  obter_nome_exercitos(m)[0] + ':' + \
                   e1_pontuacao + ' ' + obter_nome_exercitos(m)[1] + ':' + \
                   e2_pontuacao + ']' '\n' + \
    f.close()